...
const oauthUserSchema = new Schema({
  ...
  username: {
    type: String,
    unique: true,
    maxlength: [16, "User Name cannot be longer than 16 characters"],
    validate: {
      validator: (username: string) => {
        return !!username.match(/^([a-z]|[0-9])*$/i);
      },
      message: "Username can only container latin characters and numbers.",
    },
  },
});

oauthUserSchema.post("save", (err: any, doc: any, next: any) => {
  if (err.code === 11000) {
